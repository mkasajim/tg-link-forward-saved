    ...
    if urls:
        for url in urls:
            ...
            if not is_domain_blacklisted(blacklist_conn, domain):
                ...
                # Check if the full URL is already blocked
                if not full_link_exists(full_links_conn, url):
                    print("Full link does not exist")
...
async def show_blacklist(event):
    ...
    if sender_id == admin_chat_id:
        blacklist_domains = list_blacklist(blacklist_conn)
        blocked_links = list_links(full_links_conn)  # Get blocked full links

        if blacklist_domains or blocked_links:
            message = "**Blacklisted Domains:**\n"
            if blacklist_domains:
                message += "\n".join([f"{domain[0]}" for domain in blacklist_domains])
            else:
                message += "None\n"

            message += "\n**Blocked Links:**\n"
            if blocked_links:
                message += "\n".join([f"{link[0]}" for link in blocked_links])
            else:
                message += "None"
...
# Database for storing full links
full_links_db_name = 'full_links.db'

def create_full_links_connection(db_file):
    conn = None
    try:
        conn = sqlite3.connect(db_file)
    except sqlite3.Error as e:
...
def create_full_links_table(conn, create_table_sql):
    try:
        c = conn.cursor()
        c.execute(create_table_sql)
    except sqlite3.Error as e:
...
def full_link_exists(conn, full_url):
    """ Check if a full URL exists in the database """
    sql = '''SELECT id FROM full_links WHERE full_url=?'''
    cur = conn.cursor()
    cur.execute(sql, (full_url,))
    data = cur.fetchone()
    print(f"Does full link exist: {full_url}: {data is not None}")
    return data is not None

def insert_full_link(conn, full_url):
    if not full_link_exists(conn, full_url):
        sql = '''INSERT INTO full_links(full_url) VALUES(?)'''
        try:
            c = conn.cursor()
            c.execute(sql, (full_url,))
            conn.commit()
            return c.lastrowid
        except sqlite3.IntegrityError:
            return None
...
full_links_conn = create_full_links_connection(full_links_db_name)
if full_links_conn is not None:
    create_full_links_table_sql = """ CREATE TABLE IF NOT EXISTS full_links (
                                        id integer PRIMARY KEY,
                                        full_url text NOT NULL UNIQUE
                                    ); """
    create_full_links_table(full_links_conn, create_full_links_table_sql)

@bot_client.on(events.NewMessage(incoming=True, pattern='/blocklink (.+)'))
async def block_full_link(event):
    sender = await event.get_sender()
    sender_id = sender.id
    if sender_id == admin_chat_id:
        full_url_to_block = event.pattern_match.group(1)
        if not full_link_exists(full_links_conn, full_url_to_block):
            insert_full_link(full_links_conn, full_url_to_block)
            await event.reply(f"Full URL {full_url_to_block} has been blocked.")
        else:
            await event.reply(f"Full URL {full_url_to_block} is already blocked.")



@bot_client.on(events.NewMessage(incoming=True, pattern='/unblocklink (.+)'))
async def unblock_full_link(event):
    sender = await event.get_sender()
    sender_id = sender.id
    if sender_id == admin_chat_id:
        full_url_to_unblock = event.pattern_match.group(1)
        if full_link_exists(full_links_conn, full_url_to_unblock):
            sql = '''DELETE FROM full_links WHERE full_url=?'''
            try:
                c = full_links_conn.cursor()
                c.execute(sql, (full_url_to_unblock,))
                full_links_conn.commit()
                await event.reply(f"Full URL {full_url_to_unblock} has been unblocked.")
            except sqlite3.Error as e:
                print(e)
                await event.reply("An error occurred while unblocking the URL.")
        else:
            await event.reply(f"Full URL {full_url_to_unblock} is not blocked.")
