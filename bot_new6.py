...
import re
import sqlite3
from urllib.parse import urlparse, urlunparse, parse_qs, urlencode, urlunsplit
import socket
import time
...
def clean_url(url):
    ...
    url = url.strip()
    # Remove any trailing punctuation or 'N' or 'n'
    url = re.sub(r'[^\w/:.-]+$', '', url)
    url = re.sub(r'[Nn]$', '', url)
    # Fix incorrect protocol
    url = re.sub(r'^https?:\/\/tps:\/\/', 'https://', url)
...
async def user_message_handler(event):
    ...
    urls.update(map(clean_url, additional_urls))

    # Filter out invalid URLs
    valid_urls = {url for url in urls if re.match(r'^https?:\/\/[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&\'\(\)\*\+,;=.]+$', url)}

    # Remove any URLs that contain email addresses
    valid_urls = {url for url in valid_urls if not any(email in url for email in emails)}

    # Convert set back to list for consistent ordering and remove duplicates by using a set
    unique_urls = list(valid_urls)
...
def normalize_url(url):
    ...
    params = list_refer2(refer_conn)

    # Remove referral from query parameters
    removed_query = {}
    for param in params:
        if param in query:
            removed_query[param] = query.pop(param)
    
    # Remove referral from path
    path_parts = url_parts[2].split('/')
    for param in params:
        # print(param)
        if len(path_parts) >= 2 and path_parts[-2] == param:
            path_parts = path_parts[:-2]  # Remove param and the code
            break  # Exit loop after first match
    
    url_parts[2] = '/'.join(path_parts)  # Reconstruct the path
    url_parts[4] = urlencode(query, doseq=True)  # Reconstruct the query string
    
    return urlunparse(url_parts), removed_query
...
def list_refer2(conn):
    sql = '''SELECT referrer FROM refer'''
    try:
        c = conn.cursor()
        c.execute(sql)
        return [item[0] for item in c.fetchall()]  # Extract strings from tuples
    except sqlite3.Error as e:
